        ...
        for document in self.config['documents']:
            if document.split('.')[-1] == 'txt':  # Check if the document is a .txt file
                try:
                    ...
                    docs_with_priority = [
                        Document(page_content=doc.page_content, metadata={"priority": "low"}) 
                        for doc in docs
                    ]

                    if docs_with_priority:
                        self.vectorstore.add_documents(docs_with_priority)
                        print(f"📂 Added {len(docs_with_priority)} low-priority documents from '{document}'")
        ...
        try:
            ...
            documents = [
                Document(
                    ...
                    metadata={col: str(row[col]) for col in df.columns} | {"priority": "high"}
                )
...
def retrieve_response(message, llm, priority:Literal['high', 'low']):

    # filters = extract_dynamic_filters(message)
    # print("FILTERS..................", filters)
    # search_kwargs = {"filter": filters} if filters else {}
    retriever = vectorstore.as_retriever(
         search_kwargs={'k':15 if priority == 'high' else 3,'filter': {'priority': priority}}
    )
    ...
    invoked_response = qa_chain.invoke({'query':message})
    print("INVOKED RESPONSE...............", invoked_response)
    response = invoked_response.get('result')
    return response or "No response available."
