...
def entity_extractor(input:ChatInput):
    ...
    entities = " ".join(list(set(ent.text for ent in doc if ent.pos_ in ['NOUN', 'PROPN', 'ADJ'])))
    print(entities)
...
def enhanced_retriever(*args):
    ...
    for doc in relevant_docs:
        ...
        if entities:
            best_match, score, _ = process.extractOne(entities, [doc.page_content], scorer=fuzz.WRatio)
