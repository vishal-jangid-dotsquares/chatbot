...
async def retrieve_response(
    ...
    try:     
        ...
        filtering_prompt = f"""
        You are an AI assistant that extracts only the most relevant  of document based on the user's query.
        You will be given multiple documents and a question. 
        Your task is to return the most relevant summary of documents with max words count {max_results}.
        If there are less than words count {max_results}, return only the available relevant ones.
        And only gives the summarize response, don't explain about your answer, it will unnecessarily extend the response.
        ...
        Now, return the most relevant summary of documents with max words count {max_results}.
        """
